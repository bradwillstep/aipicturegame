  ...
  "objects": [
    {
      ...
      "height": 0.0-1.0
        ...
        async function extractCharacterSprite(img, characterData) {
            ...
            const charX = Math.floor(img.width * characterData.x);
            const charY = Math.floor(img.height * characterData.y);
            const charWidth = Math.floor(img.width * characterData.width);
            const charHeight = Math.floor(img.height * characterData.height);
            ...
            previewCanvas.width = charWidth;
            previewCanvas.height = charHeight;
        ...
        function initializeGame(img, detectionData) {
            ...
            Events.on(render, 'afterRender', function() {
                ...
                if (player && playerSprite) {
                    ...
                    const bobOffset = isWalking ? Math.sin(walkCycle) * 2 : 0;
                    
                    // Draw sprite centered on physics body
                    const spriteWidth = playerSprite.width * 0.3;
                    const spriteHeight = playerSprite.height * 0.3;
            ...
            let playerStartY = canvasHeight - 150;
            let playerSize = 40;

            if (detectionData && detectionData.character) {
                playerStartX = Math.max(100, canvasWidth * detectionData.character.x);
                playerStartY = Math.max(100, canvasHeight * detectionData.character.y);
                playerSize = Math.max(30, Math.min(80, canvasWidth * detectionData.character.width * 0.5));
            }

            player = Bodies.rectangle(playerStartX, playerStartY, playerSize, playerSize * 1.5, {
                restitution: 0.1,
                friction: 0.8,
                frictionAir: 0.02,
                density: 0.002,
